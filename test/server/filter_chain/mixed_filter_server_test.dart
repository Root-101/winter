...
void main() {
      ...
      router: WinterRouter(
        routes: [
          ...
          Route(
            ...
            method: HttpMethod.get,
            handler: (request) => ResponseEntity.ok(
              body: request.headers['X-Custom-Header'],
            ),
          ),
          Route(
            path: '/short-circuit',
            ...
            method: HttpMethod.get,
            handler: (request) => ResponseEntity.ok(body: 'Should not reach here'),
          ),
          Route(
            path: '/multi-filter',
            ...
            method: HttpMethod.get,
            handler: (request) => ResponseEntity.ok(body: request.headers['x-trace']),
          ),
          Route(
            path: '/post-filter',
  ...
  group('Global Filter Tests', () {
    test('Should return 401 when Authorization header is missing (Route 1)', () async {
      String urlToTest = '/mixed-filter/55';
      http.Response response = await http.get(url(urlToTest));
      expect(response.statusCode, 401);
      expect(response.body, 'Request need the ${HttpHeaders.authorization} header');
    });

    test('Should return 401 when Authorization header is missing (Route 2)', () async {
  ...
  group('Mixed Filter Tests (Global + Route)', () {
    ...
    test('Should remove params when RemoveQueryParamsFilter is present', () async {
  ...
  group('Advanced Filter Tests', () {
    ...
    test('Should execute multiple filters in the order they are defined', () async {
      http.Response response = await http.get(
        url('/multi-filter'),
        headers: authHeaders,
      );
    ...
    test('Should correctly handle path params with special characters', () async {
...
class PostFilter implements Filter {
  ...
  Future<ResponseEntity> doFilter(RequestEntity request, FilterChain chain) async {
    ...
    return response.copyWith(headers: {...response.headers, 'X-Post-Filter': 'true'});
  }
}
